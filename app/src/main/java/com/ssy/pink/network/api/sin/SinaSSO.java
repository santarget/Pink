...
import com.ssy.pink.network.api.sin.script.Invocable;
import com.ssy.pink.network.api.sin.script.ScriptEngine;
import com.ssy.pink.network.api.sin.script.ScriptEngineFactory;
import com.ssy.pink.network.api.sin.script.ScriptEngineManager;
import com.ssy.pink.network.api.sin.script.ScriptException;
...
public class SinaSSO {
    ...
    public static String getServertimeUrl = "https://login.sina.com.cn/sso/prelogin.php?entry=weibo&callback=sinaSSOController.preloginCallBack&su=MTgzMTI0OTMxMDc%3D&rsakt=mod&checkpin=1&client=ssologin.js(v1.4.19)&_=";

    public static String url = "https://login.sina.com.cn/sso/login.php?client=ssologin.js(v1.4.19)&_=";
    ...
    public void test(String accout, String pwd) {
        try {
            SetConfig config = getServerInfo();
//            downloadImage(config);
            login(accout, pwd, config);
        } catch (IOException e) {
            e.printStackTrace();
    ...
    private void downloadImage(SetConfig config) throws Exception {
        HttpClient httpClient = new DefaultHttpClient();
        for (int i = 0; i < 10; i++) {
            String url = config.getSmsurl();
            HttpGet getMethod = new HttpGet(url);
//            try {
            HttpResponse response = httpClient.execute(getMethod, new BasicHttpContext());
            HttpEntity entity = response.getEntity();
            InputStream instream = entity.getContent();
            OutputStream outstream = new FileOutputStream(new File(Environment.getExternalStorageDirectory() + "/1/", i + ".png"));
            int l = -1;
...
//            }
    ...
    private void login(String name, String password, SetConfig info) throws ScriptException, NoSuchMethodException, ClientProtocolException, IOException {
        ...
        list.add(new BasicNameValuePair("servertime", info.getServertime()));
        list.add(new BasicNameValuePair("nonce", info.getNonce()));
        list.add(new BasicNameValuePair("pwencode", "rsa2"));
        list.add(new BasicNameValuePair("rsakv", info.getRsakv()));
        ...
        if (response != null) {
            ...
            if (resEntity != null) {
                ...
                if (!TextUtils.isEmpty(result)) {
                    weiboLoginInfo = JsonUtils.toObject(result, WeiboLoginInfo.class);
                }
            }
        }
    ...
    public void repost() {
        ...
        System.out.println("crossDomainUrlList = " + weiboLoginInfo.getCrossDomainUrlList());

        List<String> urlList = weiboLoginInfo.getCrossDomainUrlList();
        ...
        try {
            ...
            System.out.println("sso info = " + EntityUtils.toString(getssoresponse.getEntity(), "UTF-8"));
//({"result":true,"userinfo":{"uniqueid":"6636712991","displayname":"_nuoxiya2002"}});
            ...
            Log.i("aaaa", "repost exception:" + e.getMessage());
            e.printStackTrace();
        }
    }
     ...
     * @throws IOException
     */
    public static SetConfig getServerInfo() throws ClientProtocolException, IOException {
        SetConfig info = new SetConfig();
...
//        info.setRsakv(configInfo.getString("rsakv"));
//        info.setServertime(configInfo.getString("servertime"));
//        info.setPcid(configInfo.getString("pcid"));
//        info.setPubkey(configInfo.getString("pubkey"));
//        info.setExectime(configInfo.getString("exectime"));
//        return info;
    }
